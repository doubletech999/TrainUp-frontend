...
function goToNotificationsPage() {
    ...
    if (userType === 'STUDENT') {
        notificationsPath = 'pages/student/notifications.html';
    } else if (userType === 'COMPANY') {
        notificationsPath = 'pages/company/notifications.html';
    } else if (userType === 'ADMIN') {
        notificationsPath = 'pages/admin/notifications.html';
    } else if (userType === 'SUPERVISOR') {
        notificationsPath = 'pages/supervisor/notifications.html';
    } else {
        ...
        if (currentPath.includes('/student/')) {
            ...
            // Default to student if path doesn't match
            notificationsPath = 'pages/student/notifications.html';
        }
    }
    
    // Check if we're already in a subdirectory and adjust path
    const currentPath = window.location.pathname;
    if (currentPath.includes('/pages/')) {
        // We're in a subdirectory, use relative path
        if (currentPath.includes('/student/')) {
            notificationsPath = 'notifications.html';
        } else if (currentPath.includes('/company/')) {
            notificationsPath = 'notifications.html';
...
async function loadNotificationsDropdown() {
    const notificationsList = getDropdownListElement();
...
function ensureDropdownListHasUniqueId() {
    const dropdown = document.getElementById('notificationsDropdown');
    if (!dropdown) return;

    const list = dropdown.querySelector('.notifications-list');
    if (list && list.id !== 'notificationsDropdownList') {
        list.id = 'notificationsDropdownList';
    }
}

function getDropdownListElement() {
    ensureDropdownListHasUniqueId();
    return document.getElementById('notificationsDropdownList');
}

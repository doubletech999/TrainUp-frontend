// ===== TrainUp - Admin Notifications JavaScript =====
...
function loadUserInfo(userData) {
    ...
    if (userNameEl) userNameEl.textContent = userData.name || 'Administrator';
    if (userEmailEl) userEmailEl.textContent = userData.email;
    if (userAvatarEl) userAvatarEl.textContent = (userData.name || 'A').charAt(0).toUpperCase();
}
    ...
    return [
        {
            ...
            message: 'A new company "TechCorp" has registered and is pending approval.',
            isRead: false,
            createdAt: new Date().toISOString(),
            actionUrl: 'users.html',
            metadata: {
                userId: 'user1',
        ...
        },
        {
            ...
            message: 'A new internship "Software Developer Intern" has been posted and requires admin review.',
            isRead: false,
        ...
        },
        {
            ...
            type: 'SYSTEM',
            title: 'System Alert',
            message: 'High number of user registrations detected today. Consider reviewing user activity.',
            isRead: true,
        ...
        },
        {
            ...
            message: '50 new applications were submitted in the last 24 hours.',
            isRead: true,
        ...
        },
        {
            ...
            message: 'User account "company@example.com" has been suspended due to policy violation.',
            isRead: true,
...
function openNotification(notificationId) {
    ...
    if (notification.actionUrl && actionBtn) {
        ...
        actionBtn.onclick = () => {
            ...
            const destination = resolveNotificationActionUrl(notification.actionUrl);
            if (destination) {
...
function resolveNotificationActionUrl(rawUrl) {
    ...
    if (url.startsWith('pages/') || url.startsWith('/pages/')) {
        return `${origin}${frontendBase}${normalize(url)}`;
    }
    ...
    // Relative paths (e.g., users.html) → stay inside current directory (already under /trainup-frontend/pages/admin/)
    return url;
}
